...
const ClientsPage: React.FC = () => {
  ...
  const updateClientMutation = useMutation({
    ...
    onError: (error: any) => {
      const errorMessage = error.response?.data?.error || error.response?.data?.details || '클라이언트 수정에 실패했습니다'
      toast.error(`클라이언트 수정 실패: ${errorMessage}`)
    },
  })
  ...
  // 서버에서 이미 필터링된 클라이언트를 사용
  const filteredClients = clients

  // 선택된 테넌트가 없으면 첫 번째 테넌트 자동 선택
  React.useEffect(() => {
    if (Array.isArray(tenants) && tenants.length > 0 && !selectedTenantId) {
  ...
  const handleEdit = (client: Client) => {
    ...
    // 폼 데이터 미리 채우기
    editSetValue('name', client.name)
    editSetValue('description', client.description || '')
  ...
  const onEditSubmit = (data: CreateClientFormData) => {
        ...
        <div className="flex items-center space-x-4">
          ...
          >
            {tenantsLoading ? (
              ...
              Array.isArray(tenants) && tenants.map((tenant: Tenant) => (
                  ...
                  <div className="flex-1">
                    ...
                    {client.post_logout_redirect_uris && client.post_logout_redirect_uris.length > 0 && (
                      <div className="mt-4">
                        ...
                        <dd className="mt-1">
                          {client.post_logout_redirect_uris.map((uri, index) => (
                            <span
                              key={index}
                              className="inline-block mr-2 mb-1 px-2 py-1 text-xs bg-purple-100 text-purple-800 rounded"
                            >
                              {uri}
                            </span>
                          ))}
          ...
          <div className="relative top-20 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white">
            <div className="mt-3">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700">
                    Post-Logout Redirect URIs (선택사항)
                  </label>
                  <textarea
      ...
      {showEditModal && editingClient && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white">
            <div className="mt-3">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium text-gray-900">클라이언트 수정</h3>
                <button
                  onClick={() => {
                    setShowEditModal(false)
                    setEditingClient(null)
              ...
              <form onSubmit={editHandleSubmit(onEditSubmit)} className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      {...editRegister('website')}
                      type="url"
                      className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                      placeholder="https://example.com"
                    />
                    {editErrors.website && (
                      <p className="mt-1 text-sm text-red-600">{editErrors.website.message}</p>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  {editErrors.redirect_uris && (
                    <p className="mt-1 text-sm text-red-600">{editErrors.redirect_uris.message}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <textarea
                    {...editRegister('post_logout_redirect_uris')}
                    rows={3}
                    className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                    placeholder={`http://localhost:3000\nhttps://example.com`}
                  />
                ...
                <div>
                  ...
                  {editErrors.grant_types && (
                    <p className="mt-1 text-sm text-red-600">{editErrors.grant_types.message}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {editErrors.scopes && (
                    <p className="mt-1 text-sm text-red-600">{editErrors.scopes.message}</p>
                  )}
                </div>
                ...
                <div className="flex justify-end space-x-3 pt-4">
                  ...
                  <button
                    ...
                    className="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50"
                  >
                    {updateClientMutation.isPending ? '수정 중...' : '수정'}
                  </button>
                </div>
              </form>
            </div>
