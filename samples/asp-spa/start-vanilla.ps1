...
Write-Host "  🚀 SPA Sample - Vanilla JS Version" -ForegroundColor Cyan
Write-Host "═══════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
...
# ============================================================
# 4. Start Vanilla Frontend
# ============================================================

if (-not $SkipFrontend) {
    Write-Host "═══════════════════════════════════════════" -ForegroundColor Cyan
    Write-Host "  ⚡ Starting Vanilla JS Frontend" -ForegroundColor Cyan
    Write-Host "═══════════════════════════════════════════" -ForegroundColor Cyan
    Write-Host ""

    $frontendPath = Join-Path $ScriptDir "asp-frontend-vanilla"
    if (Test-Path $frontendPath) {
        Write-Host "Starting frontend at http://localhost:$FRONTEND_PORT..." -ForegroundColor Cyan
        ...
        # Start Vite dev server on custom port
        Start-Process pwsh -ArgumentList "-NoExit", "-Command", "cd '$frontendPath'; npm run dev -- --port $FRONTEND_PORT" -WorkingDirectory $frontendPath
...
Write-Host "═══════════════════════════════════════════" -ForegroundColor Green
Write-Host "  ✅ SPA Sample (Vanilla JS) Started!" -ForegroundColor Green
Write-Host "═══════════════════════════════════════════" -ForegroundColor Green
Write-Host ""
Write-Host "📌 Application URLs:" -ForegroundColor Cyan
Write-Host "  Vanilla Frontend: http://localhost:$FRONTEND_PORT" -ForegroundColor White
...
Write-Host ""
Write-Host "💡 Features (Vanilla JavaScript):" -ForegroundColor Yellow
Write-Host "  • Pure JavaScript (no framework)" -ForegroundColor White
Write-Host "  • OAuth 2.0 + PKCE with oauth4webapi" -ForegroundColor White
Write-Host "  • Popup Login 🪟 (with postMessage)" -ForegroundColor White
Write-Host "  • Dynamic Claims 🎭" -ForegroundColor White
Write-Host "  • Token Management 🎫" -ForegroundColor White
Write-Host "  • Protected API calls 🔌" -ForegroundColor White
Write-Host ""
